    ...
    for vol_name in sorted(vol_grouped.keys()):
        ...
        for task in vol_grouped[vol_name]:
            ...
            if task['start'] == VOL_STARTS.get(vol_name):
                pdf_start = 0
            elif task['start'] in marker_map:
                pdf_start = marker_map[task['start']][0]
            else:
            ...
            if task['end'] in marker_map:
                pdf_end = marker_map[task['end']][0]
            else:
                for lookahead in range(1, 15):
                    cand = task['end'] + lookahead
                    if cand in marker_map:
                        pdf_end = marker_map[cand][0]
                        break
                else:
                    print(f"    [SKIP] No marker found near end page {task['end']}")
                    continue
